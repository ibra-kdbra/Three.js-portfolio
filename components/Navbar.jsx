...
function Navbar() {
  ...
  function AvatarModal() {
    return (
      <aside className="w-[100svw] h-[100svh] flex justify-center items-center bg-[#0000007a] fixed top-0 left-0">
        <div
          className="sm:w-[500px] sm:h-[500px] xs:w-[400px] xs:h-[400px] w-[200px] h-[200px] 
				dark:bg-[#2b2b42d2] bg-[#e0eaf0] flex justify-center 
				items-center backdrop-blur-sm backdrop-filter bg-opacity-80 rounded-md modal"
        >
          <div className="relative w-[80%] h-[80%] rounded-md">
            ...
            <div
              className="w-[28px] h-[28px] object-contain text-ctnPrimaryLight dark:text-ctnPrimaryDark 
							flex justify-center items-center cursor-pointer absolute top-[-30px] right-[-30px]"
  ...
  return (
    <>
      <nav
        className={`paddingX w-full flex items-center py-5 fixed top-0 z-30 bg-transparent backdrop-filter 
      ...
      >
        ...
        <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
          <div href="/" className="flex items-center gap-6">
            ...
            <Link href="/">
              <p
                className="dark:text-ctnPrimaryDark text-ctnPrimaryLight text-[18px] font-bold 
							cursor-pointer flex "
              >
                Farouq Khankan &nbsp;
                <span className="lg:block hidden"> | Web Application Engineer</span>
              </p>
            </Link>
          </div>
          ...
          <ul className="list-none hidden md:flex flex-row gap-10 items-center">
            {navLinks.map((nav) => (
              <li
                key={nav.id}
                className={`dark:text-ctnPrimaryDark text-ctnPrimaryLight border-secondary 
									transition-all duration-200 ease-in text-[18px] font-medium cursor-pointer ${
                    active === nav.title
          ...
          <div className="md:hidden flex flex-1 justify-end items-center">
            ...
            <motion.div
              ...
              className={`${
                !toggle ? "hidden" : "flex"
              } p-6 bg-bgSecondaryLight dark:bg-bgSecondaryDark absolute top-20 right-0 mx-4 my-2 min-w-[140px] 
							 z-10 rounded-xl`}
            >
